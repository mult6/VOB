...
class WebSocketPriceProvider:
    """
    Адаптер OpinionWebSocket для использования в Telegram боте
    Предоставляет интерфейс совместимый с auto_trader.py
    """
    
    def __init__(self, api_key: str = None):
        """
        Создать провайдер цен
        
        Args:
            api_key: API ключ для WebSocket
        """
        self.api_key = api_key
        self._ws: Optional[OpinionWebSocket] = None
        self._subscribed_markets: Dict[int, Dict] = {}  # market_id -> {yes_token, no_token}
        self._started = False
        self._lock = threading.Lock()
    
    async def start(self):
        """Запустить WebSocket (async интерфейс для совместимости)"""
        with self._lock:
            if self._started:
                return
            
            self._ws = OpinionWebSocket(api_key=self.api_key)
            if self._ws.connect():
                self._started = True
                logger.info("✅ WebSocketPriceProvider started")
            else:
                logger.error("❌ WebSocketPriceProvider failed to connect")
                self._ws = None
    ...
    async def stop(self):
        ...
        with self._lock:
            ...
            logger.info("🔌 WebSocketPriceProvider stopped")
    
    async def subscribe_market(self, market_id: int, yes_token_id: str, no_token_id: str):
        """
        Подписаться на рынок
        
        Args:
            market_id: ID рынка
            yes_token_id: Token ID для YES
            no_token_id: Token ID для NO
        """
        with self._lock:
            if not self._ws:
                logger.error("WebSocket not started")
                return
            
            # Запоминаем token IDs для этого рынка
            self._subscribed_markets[market_id] = {
                'yes': yes_token_id,
                'no': no_token_id
            }
            
            # Подписываемся через OpinionWebSocket
            self._ws.subscribe_orderbook(market_id)
            logger.info(f"📊 Subscribed to market {market_id}")
    
    def get_prices(self, market_id: int) -> tuple:
        """
        Получить цены для рынка
        
        Args:
            market_id: ID рынка
            
        Returns:
            (yes_bid, yes_ask, no_bid, no_ask) или (None, None, None, None)
        """
        with self._lock:
            if not self._ws:
            ...
            # Получаем token IDs
            tokens = self._subscribed_markets.get(market_id)
            if not tokens:
                # Пробуем из кэша WebSocket
                tokens = self._ws._market_tokens.get(market_id)
                if not tokens:
                    logger.warning(f"No tokens for market {market_id}")
                    return None, None, None, None
            ...
            return self._ws.get_market_prices(yes_token, no_token)
